...
- Déploiement une application .NET en exécutable Windows

---

## 8. Optionnel : Ajout de la langue Wolof

La langue **Wolof** a été ajoutée à l'application en tant que langue supplémentaire, tout en conservant les options de culture du français.

### Modifications effectuées :

1.  **Configuration (`Startup.cs`)** : Ajout de `new CultureInfo("wo")` à la liste des cultures supportées.
2.  **Fichiers de ressources créés** :
    - `Resources/Views/Shared/Components/CartSummary/Default.wo.resx` : Traduction du résumé du panier ("Sa Panier").
    - `Resources/Models/ViewModels/LanguageViewModel.wo.resx` : Traduction des noms de langues.
    - `Resources/Controllers/OrderController.wo.resx` : Messages du contrôleur de commande.
    - `Resources/Models/ViewModels/Order.wo.resx` : Messages de validation du formulaire de commande.
3.  **Mise à jour des ressources existantes** : Ajout de l'option "Wolof" dans les fichiers `LanguageViewModel` anglais et français.

### Résultat :
L'utilisateur peut désormais sélectionner "Wolof" dans le menu de sélection de la langue, et l'interface s'adapte avec les traductions fournies.
```
